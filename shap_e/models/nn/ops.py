...
class MetaLinear(MetaModule):
    def __init__(
        ...
        **kwargs,
        ...
        register_proj_fn = register_meta_fn if meta_proj else self.register_parameter
        register_proj_fn("weight", nn.Parameter(torch.empty((n_out, n_in), **kwargs)))

        if not bias:
            self.register_parameter("bias", None)
        else:
            register_bias_fn = register_meta_fn if meta_bias else self.register_parameter
